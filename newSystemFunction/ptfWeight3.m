function [TableWeight,error] = ptfWeight3(TableData,type,volWin)
...
if type == 0 %eqSize
    tbl_dt=varfun(@nansum,TableData(:,{'date','time'}),'GroupingVariables',{'date','time'});
    TableWeight = outerjoin(TableData(:,{'date','time','code'}),tbl_dt,'MergeKeys',true,'Type','left');
    TableWeight.weight = 1./TableWeight.GroupCount;
elseif type ==1 %波动率倒数归一
    ...
    TableWeight = outerjoin(TableData(:,{'date','time','code','volweight'}),tbl_dt,'MergeKeys',true,'Type','left');
    TableWeight.weight = TableWeight.volweight./TableWeight.nansum_volweight;
elseif type ==2  %仅针对期货,atr倒数诡异 
    ...
    TableWeight = outerjoin(TableData(:,{'date','time','code','atrweight'}),tbl_dt,'MergeKeys',true,'Type','left');
    TableWeight.weight = TableWeight.atrweight./TableWeight.nansum_atrweight;
...
elseif type==4 % 等波动率，股票期货均可
    ...
    TableWeight = table;
    TableWeight.date = TableData.date;
    TableWeight.time = TableData.time;
    TableWeight.code = TableData.code;
    TableWeight.weight = weight;
...
if TableData.code(1)>700000
    TableWeight = outerjoin(TableOri,TableWeight,'MergeKeys',true,'Type','left');
end
TableWeight = TableWeight(:,{'date','time','code','weight'});
TableWeight.weight(isnan(TableWeight.weight)) = 0;    
end
